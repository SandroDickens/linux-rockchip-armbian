...
static int wl_android_get_channel_list(struct net_device *dev, char *command,
				       int total_len);
...
/* wbtext need to be disabled while updating roam/scan parameters */
	...
	{ CMD_SETOKCMODE, DEFAULT_OKCMODE, RESTORE_TYPE_PRIV_CMD,
	  .cmd_handler = wl_android_set_okc_mode },
#endif /* WES_SUPPORT */
...
static int wl_android_set_channel_width(struct net_device *dev, char *command,
					int total_len)
{
	u32 channel_width = 0;
	struct wireless_dev *wdev = dev->ieee80211_ptr;
	struct bcm_cfg80211 *cfg =
		(struct bcm_cfg80211 *)wiphy_priv(wdev->wiphy);
...
static int wl_android_hogsqs(struct net_device *dev, char *command,
			     int total_len)
{
			...
			if (reg == M_HOGSQS_TXCFE_DET_CNT)
				bytes_written =
					snprintf(command, total_len,
						 " %s 0x%x/0x%x", CMD_AP_HOGSQS,
						 (value & 0x00FF),
						 ((value & 0xFF00) >> 8));
			else
				bytes_written =
					snprintf(command, total_len, " %s 0x%x",
						 CMD_AP_HOGSQS, value);
...
static int wl_android_scan_protect_bss(struct net_device *dev, char *command,
				       int total_len)
{
...
#ifdef DHD_BANDSTEER
static int wl_android_set_bandsteer(struct net_device *dev, char *command,
				    int total_len)
{
	char *iftype;
	...
	struct wireless_dev *__wdev =
		(struct wireless_dev *)(dev)->ieee80211_ptr;
	struct bcm_cfg80211 *cfg =
		(struct bcm_cfg80211 *)wiphy_priv(__wdev->wiphy);
	...
	if (!strncmp(iftype, "p2p", 3)) {
		...
		if (val) {
			ret = dhd_bandsteer_module_init(dev, cfg->ap_bs,
							cfg->p2p_bs);
			if (ret == BCME_ERROR) {
				DHD_ERROR(
					("%s: Failed to enable %s bandsteer\n",
					 __FUNCTION__,
					 cfg->ap_bs ? "ap" : "p2p"));
				return ret;
			} else {
				DHD_ERROR((
					"%s: Successfully enabled %s bandsteer\n",
					__FUNCTION__,
					cfg->ap_bs ? "ap" : "p2p"));
			}
		} else {
			ret = dhd_bandsteer_module_deinit(dev, cfg->ap_bs,
							  cfg->p2p_bs);
			if (ret == BCME_ERROR) {
				DHD_ERROR(
					("%s: Failed to disable %s bandsteer\n",
					 __FUNCTION__,
					 cfg->ap_bs ? "ap" : "p2p"));
				return ret;
			} else {
				DHD_ERROR((
					"%s: Successfully disabled %s bandsteer\n",
					__FUNCTION__,
					cfg->ap_bs ? "ap" : "p2p"));
			}
		}
...
static int wl_android_set_maxassoc_limit(struct net_device *dev, char *command,
					 int total_len)
{
	...
	*/
	if (delim) {
		...
		*/
		...
		scbval.val = htod32(0);
		DHD_TRACE(("wl_android_get_rssi: address:" MACDBG,
...
int wl_android_get_rsdb_mode(struct net_device *dev, char *command,
			     int total_len)
{
	int bytes_written = 0;
				       ...
				       int total_len)
{
	...
	error = wldev_ioctl_get(dev, WLC_GET_CHANNELS_IN_COUNTRY, cic,
				sizeof(smbuf));
	if (error) {
...
int wl_android_block_associations(struct net_device *dev, char *command,
				  int total_len)
{
	int enable_blockassoc = 0, bytes_written = 0, ret = 0;

	if (*(command + strlen(CMD_BLOCKASSOC)) == '\0') {
		ret = wldev_iovar_getint(dev, "block_assoc",
					 &enable_blockassoc);
		if (ret != BCME_OK) {
			DHD_ERROR((
				"%s: Failed to get status of block_assoc error(%d)\n",
				__FUNCTION__, ret));
			return ret;
		}
		bytes_written = snprintf(command, total_len, "%s %d",
					 CMD_BLOCKASSOC, enable_blockassoc);
		return bytes_written;
	}
...
#if !defined(DISALBE_11H) && defined(DHD_NOSCAN_DURING_CSA)
	cfg->in_csa = TRUE;
	mod_timer(&cfg->csa_timeout,
		  jiffies + msecs_to_jiffies(100 * (csa_arg.count + 2)));
#endif // endif
	return 0;
}
...
static int wl_android_add_vendor_ie(struct net_device *dev, char *command,
				    int total_len)
{
...
static int wl_android_del_vendor_ie(struct net_device *dev, char *command,
				    int total_len)
{
	char ie_buf[VNDR_IE_MAX_LEN];
	char *ioctl_buf = NULL;
				      ...
				      int total_len)
{
	...
	channel_cnt =
		get_roamscan_channel_list(dev, channels, MAX_ROAM_CHANNEL);
	bytes_written = snprintf(command, total_len, "%s %d",
...
int wl_android_set_roam_scan_channels(struct net_device *dev, char *command)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_roam_scan_channels: Failed to set Scan Channels %d,"
			" error = %d\n",
			p[0], error));
		return -1;
	}
				     ...
				     int total_len)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_scan_channel_time: Failed to get Scan Channel Time,"
			" error = %d\n",
			error));
		return -1;
	}
...
int wl_android_set_scan_channel_time(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &time) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_channel_time: Failed to get Parameter\n"));
		return -1;
	}
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_channel_time: Failed to set Scan Channel Time %d,"
			" error = %d\n",
			time, error));
		return -1;
	}
				     ...
				     int total_len)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_scan_unassoc_time: Failed to get Scan Unassoc"
			" Time, error = %d\n",
			error));
		return -1;
	}
...
int wl_android_set_scan_unassoc_time(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &time) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_unassoc_time: Failed to get Parameter\n"));
		return -1;
	}
#ifdef CUSTOMER_SCAN_TIMEOUT_SETTING
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_unassoc_time: Failed to set Scan Unassoc Time %d,"
			" error = %d\n",
			time, error));
		return -1;
	}
				     ...
				     int total_len)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_scan_passive_time: Failed to get Scan Passive Time,"
			" error = %d\n",
			error));
		return -1;
	}
...
int wl_android_set_scan_passive_time(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &time) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_passive_time: Failed to get Parameter\n"));
		return -1;
	}
#ifdef CUSTOMER_SCAN_TIMEOUT_SETTING
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_passive_time: Failed to set Scan Passive Time %d,"
			" error = %d\n",
			time, error));
		return -1;
	}
...
int wl_android_set_scan_home_time(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &time) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_home_time: Failed to get Parameter\n"));
		return -1;
	}
#ifdef CUSTOMER_SCAN_TIMEOUT_SETTING
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_home_time: Failed to set Scan Home Time %d,"
			" error = %d\n",
			time, error));
		return -1;
	}
				       ...
				       int total_len)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_scan_home_away_time: Failed to get Scan Home Away Time,"
			" error = %d\n",
			error));
		return -1;
	}
...
int wl_android_set_scan_home_away_time(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &time) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_home_away_time: Failed to get Parameter\n"));
		return -1;
	}
#ifdef CUSTOMER_SCAN_TIMEOUT_SETTING
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_home_away_time: Failed to set Scan Home Away"
			" Time %d, error = %d\n",
			time, error));
		return -1;
	}
				...
				int total_len)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_scan_nprobes: Failed to get Scan NProbes,"
			" error = %d\n",
			error));
		return -1;
	}
...
int wl_android_set_scan_nprobes(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &num) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_nprobes: Failed to get Parameter\n"));
		return -1;
	}
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_nprobes: Failed to set Scan NProbes %d,"
			" error = %d\n",
			num, error));
		return -1;
	}
					 ...
					 int total_len)
{
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_scan_dfs_channel_mode: Failed to get Passive Time,"
			" error = %d\n",
			error));
		return -1;
	}
...
int wl_android_set_scan_dfs_channel_mode(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &mode) != 1) {
		DHD_ERROR((
			"wl_android_set_scan_dfs_channel_mode: Failed to get Parameter\n"));
		return -1;
	}
	...
	if (mode == 1) {
		...
		DHD_ERROR((
			"wl_android_set_scan_dfs_channel_mode: Failed to set Scan DFS"
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_scan_dfs_channel_mode: Failed to set Scan"
...
static int wl_android_set_join_prefer(struct net_device *dev, char *command)
{
	...
	if (wl_android_check_wbtext(dev)) {
		...
		if ((error = wl_android_wbtext(dev, commandp,
					       WLC_IOCTL_SMLEN)) != BCME_OK) {
...
int wl_android_reassoc(struct net_device *dev, char *command, int total_len)
{
	...
	params = (android_wifi_reassoc_params_t *)(command +
						   strlen(CMD_REASSOC) + 1);
	...
	if (wl_cfg80211_get_ioctl_version() == 1) {
		band = ((channel <= CH_MAX_2G_CHANNEL) ? WL_LCHANSPEC_BAND_2G :
							 WL_LCHANSPEC_BAND_5G);
		reassoc_params.chanspec_list[0] = channel | band |
						  WL_LCHANSPEC_BW_20 |
		...
		reassoc_params.chanspec_list[0] =
			channel | band | WL_CHANSPEC_BW_20;
	}
#else
	band = ((channel <= CH_MAX_2G_CHANNEL) ? WL_CHANSPEC_BAND_2G :
						 WL_CHANSPEC_BAND_5G);
	reassoc_params.chanspec_list[0] =
		channel | band | WL_CHANSPEC_BW_20 | WL_CHANSPEC_CTL_SB_NONE;
#endif /* D11AC_IOTYPES */
...
int wl_android_get_okc_mode(struct net_device *dev, char *command,
			    int total_len)
{
	int error = 0;
	int bytes_written = 0;
	...
	if (error) {
		DHD_ERROR((
			"wl_android_get_okc_mode: Failed to get OKC Mode, error = %d\n",
			error));
		return -1;
	}

	bytes_written =
		snprintf(command, total_len, "%s %d", CMD_GETOKCMODE, mode);
...
int wl_android_set_okc_mode(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &mode) != 1) {
		DHD_ERROR(
			("wl_android_set_okc_mode: Failed to get Parameter\n"));
		return -1;
	}
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_okc_mode: Failed to set OKC Mode %d, error = %d\n",
			mode, error));
		return -1;
	}
...
#ifdef SUPPORT_RESTORE_SCAN_PARAMS
static int wl_android_restore_scan_params(struct net_device *dev, char *command,
					  int total_len)
{
	int error = 0;
	uint error_cnt = 0;
...
#ifdef CONFIG_SILENT_ROAM
int wl_android_sroam_turn_on(struct net_device *dev, const char *turn)
{
			     ...
			     int total_len)
{
	...
	if (iovar_buf == NULL) {
		DHD_ERROR((
			"wl_cfg80211_get_sta_info: failed to allocated memory %d bytes\n",
			WLC_IOCTL_MAXLEN));
		goto error;
	}
	...
	if (get_bigdata_softap) {
		...
		if (wl_get_ap_stadata(bcm_cfg, &mac, &data) == BCME_OK) {
			...
			bytes_written = snprintf(
				...
				sta_data->mode_80211, sta_data->nss,
				sta_data->mimo, sta_data->reason_code);
				       ...
				       int total_len)
{
	...
	if (cmd_str) {
		WL_INFORM(("Command: %s len:%d \n", cmd_str,
			   (int)strlen(cmd_str)));
		if (strncmp(cmd_str, APCS_BAND_AUTO, strlen(APCS_BAND_AUTO)) ==
		    0) {
			band = WLC_BAND_AUTO;
	...
	if (channel) {
		channel = (channel <= CH_MAX_2G_CHANNEL) ? channel :
							   APCS_DEFAULT_2G_CH;
		goto done2;
	}
	...
	if (band == WLC_BAND_AUTO) {
		...
		if ((ret = wl_cfg80211_get_chanspecs_5g(
			     dev, reqbuf, CHANSPEC_BUF_SIZE)) < 0) {
		...
		if ((ret = wl_cfg80211_get_chanspecs_2g(
			     dev, reqbuf, CHANSPEC_BUF_SIZE)) < 0) {
...
#endif /* D11AC_IOTYPES */
			apcs_band =
				(band == WLC_BAND_AUTO) ? WLC_BAND_2G : band;
...
static int wl_android_sta_diassoc(struct net_device *dev, const char *straddr)
{
	...
	DHD_ERROR(("wl_android_sta_diassoc: deauth STA: " MACDBG
		   " scb_val.val %d\n",
...
int wl_android_set_roam_mode(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &mode) != 1) {
		DHD_ERROR((
			"wl_android_set_roam_mode: Failed to get Parameter\n"));
		return -1;
	}
	...
	if (error) {
		DHD_ERROR((
			"wl_android_set_roam_mode: Failed to set roaming Mode %d, error = %d\n",
			mode, error));
		return -1;
	} else
		DHD_ERROR((
			"wl_android_set_roam_mode: succeeded to set roaming Mode %d,"
			" error = %d\n",
			mode, error));
	return 0;
}
				 ...
				 struct io_cfg *config)
{
	...
	if (config->iovar) {
		...
		if (ret) {
			DHD_ERROR((
				"wl_android_iolist_add: Failed to get current %s value\n",
				config->iovar));
			goto error;
		}
		...
		if (ret) {
			DHD_ERROR((
				"wl_android_iolist_add: Failed to set %s to %d\n",
...
static int wl_android_set_miracast(struct net_device *dev, char *command)
{
	...
	if (sscanf(command, "%*s %d", &mode) != 1) {
		DHD_ERROR((
			"wl_android_set_miracasts: Failed to get Parameter\n"));
		return -1;
	}
...
int wl_keep_alive_set(struct net_device *dev, char *extra)
{
	...
	if (sscanf(extra, "%d", &period_msec) != 1) {
		DHD_ERROR((
			"wl_keep_alive_set: sscanf error. check period_msec value\n"));
		return -EINVAL;
	}
	...
	if (ret < 0)
		DHD_ERROR(
			("wl_keep_alive_set:keep_alive set failed:%d\n", ret));
	else
...
static int wl_android_set_rps_cpus(struct net_device *dev, char *command)
{
	...
	if (!error) {
		...
		if (enable) {
			...
			DHD_TRACE((
				"wl_android_set_rps_cpus: clear ack suppress.\n"));
...
#define PRIVATE_COMMAND_DEF_LEN 4096
	...
	if (bytes_written >= 0) {
		...
		if (copy_to_user(priv_cmd.buf, command, bytes_written)) {
			DHD_ERROR((
				"wl_android_priv_cmd: failed to copy data to user buffer\n"));
			ret = -EFAULT;
		}
	} else {
		/* Propagate the error */
				     ...
				     struct net_device *ndev, bool user_trigger)
{
	...
	if (unlikely(err)) {
		...
		WL_INFORM_MEM(
			("bcnrecv started. user_trigger:%d\n", user_trigger));
		if (user_trigger) {
			if ((err = wl_android_bcnrecv_event(
				     ndev, BCNRECV_ATTR_STATUS,
...
int wl_handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
	...
	if (strnicmp(command, CMD_START, strlen(CMD_START)) == 0) {
		DHD_INFO((
			"wl_handle_private_cmd, Received regular START command\n"));
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		bytes_written = wl_android_add_vendor_ie(net, command,
							 priv_cmd.total_len);
	} else if (strnicmp(command, CMD_DELIE, strlen(CMD_DELIE)) == 0) {
		bytes_written = wl_android_del_vendor_ie(net, command,
							 priv_cmd.total_len);
	}
#ifdef WL_CFG80211
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
		...
		bytes_written = wl_cfg80211_set_country_code(
			net, country_code, true, false, revinfo);
...
#endif /* WL_CFG80211 */
	else if (strnicmp(command, CMD_CHANNELS_IN_CC,
			  strlen(CMD_CHANNELS_IN_CC)) == 0) {
		bytes_written = wl_android_get_channel_list(net, command,
							    priv_cmd.total_len);
		...
		bytes_written = wl_android_get_chanspec(net, command,
							priv_cmd.total_len);
	} else if (strnicmp(command, CMD_BLOCKASSOC, strlen(CMD_BLOCKASSOC)) ==
		   0) {
		bytes_written = wl_android_block_associations(
		...
		bytes_written = wl_android_get_rsdb_mode(net, command,
							 priv_cmd.total_len);
	}
#if defined(CUSTOMER_HW4_PRIVATE_CMD) || defined(IGUANA_LEGACY_CHIPS)
#ifdef ROAM_API
		...
		bytes_written = wl_android_get_roam_scan_period(
			net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_FULLROAMSCANPERIOD_SET,
			    strlen(CMD_FULLROAMSCANPERIOD_SET)) == 0) {
		bytes_written = wl_android_set_full_roam_scan_period(
			net, command, priv_cmd.total_len);
		...
		if (wldev_iovar_setint(net, "fccpwrlimit2g", FALSE)) {
			...
			DHD_ERROR((
				"wl_handle_private_cmd: fccpwrlimit2g is deactivated\n"));
		}
#endif /* FCC_PWR_LIMIT_2G */
		...
		bytes_written =
			wl_android_reassoc(net, command, priv_cmd.total_len);
		...
		bytes_written = wl_android_get_scan_nprobes(net, command,
							    priv_cmd.total_len);
		...
		bytes_written =
			wl_android_set_scan_dfs_channel_mode(net, command);
	} else if (strnicmp(command, CMD_SETJOINPREFER,
			    strlen(CMD_SETJOINPREFER)) == 0) {
		bytes_written = wl_android_set_join_prefer(net, command);
	} else if (strnicmp(command, CMD_GETWESMODE, strlen(CMD_GETWESMODE)) ==
		   0) {
		bytes_written = wl_android_get_wes_mode(net, command,
							priv_cmd.total_len);
	} else if (strnicmp(command, CMD_SETWESMODE, strlen(CMD_SETWESMODE)) ==
		   0) {
		bytes_written = wl_android_set_wes_mode(net, command);
	} else if (strnicmp(command, CMD_GETOKCMODE, strlen(CMD_GETOKCMODE)) ==
		   0) {
		bytes_written = wl_android_get_okc_mode(net, command,
							priv_cmd.total_len);
	} else if (strnicmp(command, CMD_SETOKCMODE, strlen(CMD_SETOKCMODE)) ==
		   0) {
		...
		bytes_written = wl_android_restore_scan_params(
			net, command, priv_cmd.total_len);
	}
#endif /* SUPPORT_RESTORE_SCAN_PARAMS */
#ifdef WLTDLS
		...
		bytes_written = wl_android_sroam_turn_on(
		...
		bytes_written = wl_cfg80211_get_best_channels(
			net, command, priv_cmd.total_len);
	}
#endif /* WL_SUPPORT_AUTO_CHANNEL */
#if defined(WL_SUPPORT_AUTO_CHANNEL)
	...
	/* CMD_AMPDU_MPDU */
		...
		bytes_written = wl_android_set_bandsteer(net, command,
							 priv_cmd.total_len);
	} else if (strnicmp(command, CMD_BANDSTEER_TRIGGER,
			    strlen(CMD_BANDSTEER_TRIGGER)) == 0) {
		...
		bytes_written =
			wl_android_hogsqs(net, command, priv_cmd.total_len);
	}
#endif /* ENABLE_HOGSQS */
#ifdef WL_NATOE
...
#endif /* WL_GET_CU */
	else if (strnicmp(command, CMD_CHANNEL_WIDTH,
			  strlen(CMD_CHANNEL_WIDTH)) == 0) {
		bytes_written = wl_android_set_channel_width(
			net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_TRANSITION_DISABLE,
			    strlen(CMD_TRANSITION_DISABLE)) == 0) {
		int transition_disabled =
			*(command + strlen(CMD_TRANSITION_DISABLE) + 1) - '0';
		bytes_written = wl_cfg80211_set_transition_mode(
			net, transition_disabled);
		...
		bytes_written = wl_android_set_maxassoc_limit(
			net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_SCAN_PROTECT_BSS,
			    strlen(CMD_SCAN_PROTECT_BSS)) == 0) {
		bytes_written = wl_android_scan_protect_bss(net, command,
							    priv_cmd.total_len);
	} else {
...
#ifdef WL_GENL
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 10, 0))
static int wl_genl_register_family_with_ops_groups(
	struct genl_family *family, const struct genl_ops *ops, size_t n_ops,
...
static int wl_genl_init(void)
{
	...
	ret = genl_register_family_with_ops_groups(&wl_genl_family, wl_genl_ops,
						   wl_genl_mcast);
#else
	ret = wl_genl_register_family_with_ops_groups(
		&wl_genl_family, wl_genl_ops, ARRAY_SIZE(wl_genl_ops),
		wl_genl_mcast, ARRAY_SIZE(wl_genl_mcast));
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(3, 13, 0) */
	if (ret != 0) {
				  ...
				  struct net_device *ndev)
{
	...
	wl_cfg80211_update_iflist_info(cfg, ndev, (DHD_MAX_IFS + iface_num),
				       NULL, WL_BSSIDX_MAX, NULL,
