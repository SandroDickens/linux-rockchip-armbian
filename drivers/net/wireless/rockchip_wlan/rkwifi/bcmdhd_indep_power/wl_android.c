	...
	*/
	if (delim) {
		...
		*/
		...
		scbval.val = htod32(0);
		ANDROID_TRACE(("%s: address:" MACDBG, __FUNCTION__,
			     ...
			     int total_len)
{
	...
	for (i = 0; i < assoc_maclist->count; i++) {
		bytes_written +=
			snprintf(command + bytes_written, total_len, " " MACDBG,
	...
	if (!argc) {
		...
		if (data) {
			...
			if ((error = wldev_ioctl_set(
				...
				ANDROID_ERROR((
					"%s: Failed to reset roam trigger = %d\n",
					__FUNCTION__, error));
				return error;
			}
			...
			for (i = 0; i < assoc_maclist->count; i++) {
				...
				if ((macmode == MACLIST_MODE_ALLOW && !match) ||
					...
					if ((ret = wldev_ioctl_set(
						     ...
						     sizeof(scb_val_t))) != 0)
						ANDROID_ERROR((
							"%s WLC_SCB_DEAUTHENTICATE error=%d\n",
							__FUNCTION__, ret));
				}
			}
		}
...
static int wl_android_set_mac_address_filter(struct net_device *dev, char *str)
{
	...
	if (macnum < 0 || macnum > MAX_NUM_MAC_FILT) {
		ANDROID_ERROR(
			("%s : invalid number of MAC address entries %d\n",
			 __FUNCTION__, macnum));
		return -1;
	}
	/* allocate memory for the MAC list */
	list = (struct maclist *)kmalloc(
		sizeof(int) + sizeof(struct ether_addr) * macnum, GFP_KERNEL);
	if (!list) {
		ANDROID_ERROR(
	...
	for (i = 0; i < list->count; i++) {
		...
		if (!(ret = bcm_ether_atoe(eabuf, &list->ea[i]))) {
			ANDROID_ERROR(
				("%s : mac parsing err index=%d, addr=%s\n",
				 __FUNCTION__, i, eabuf));
			list->count--;
			break;
		}
		ANDROID_INFO(("%s : %d/%d MACADDR=%s", __FUNCTION__, i,
			      list->count, eabuf));
	}
	/* set the list */
...
int wl_android_wifi_on(struct net_device *dev)
{
	...
	if (!g_wifi_on) {
		do {
			if (!dhd_net_wifi_platform_set_power(
				    dev, TRUE, WIFI_TURNON_DELAY)) {
#ifdef BCMSDIO
					   ...
					   char *command, int total_len)
{
	...
	if (bufsize < sizeof(struct connection_stats)) {
		ANDROID_ERROR((
			"%s: not enough buffer size, provided=%u, requires=%zu\n",
	...
	if (ret) {
		...
		if (dtoh16(if_stats->version) > WL_IF_STATS_T_VERSION) {
			ANDROID_ERROR((
				"%s: incorrect version of wl_if_stats_t, expected=%u got=%u\n",
	...
	if (ret || link_speed < 0) {
		ANDROID_ERROR((
			"%s: wldev_get_link_speed() failed, ret=%d, speed=%d\n",
				       ...
				       int total_len)
{
	...
	if (cmd_str) {
		ANDROID_INFO(("Command: %s len:%d \n", cmd_str,
			      (int)strlen(cmd_str)));
		if (strnicmp(cmd_str, APCS_BAND_AUTO, strlen(APCS_BAND_AUTO)) ==
		    0) {
			band = WLC_BAND_AUTO;
	...
	if ((ret = wldev_ioctl_get(dev, WLC_GET_SPECT_MANAGMENT, &spect,
				   sizeof(spect))) < 0) {
				   ...
				   int total_len)
{
	...
	if (error) {
		ANDROID_ERROR((
			"%s: Failed to get single core scan Mode, error = %d\n",
			__FUNCTION__, error));
		return -1;
	}
					  ...
					  const char *string_num)
{
	...
	if (mode == 0) {
		ANDROID_INFO(
			("%s: SAR limit control activated\n", __FUNCTION__));
		setval = 1;
	} else if (mode == -1) {
		ANDROID_INFO(
			("%s: SAR limit control deactivated\n", __FUNCTION__));
		setval = 0;
	} else {
		return -EINVAL;
	}
...
#ifdef WL11ULB
static int wl_android_set_ulb_mode(struct net_device *dev, char *command,
				   int total_len)
{
	int mode = 0;
...
static int wl_android_set_ulb_bw(struct net_device *dev, char *command,
				 int total_len)
{
	int bw = 0;
	u8 *pos;
...
#ifdef BT_WIFI_HANDOVER
static int wl_tbow_teardown(struct net_device *dev, char *command,
			    int total_len)
{
	int err = BCME_OK;
				...
				int total_len)
{
	...
	ANDROID_TRACE(("ifname %s, version %d, count %d, mimo rssi %d\n",
		       ifname, rssi_ant_mimo.version, rssi_ant_mimo.count,
		       mimo_rssi));
	if (mimo_rssi) {
...
#ifdef SET_PCIE_IRQ_CPU_CORE
void wl_android_set_irq_cpucore(struct net_device *net, int set)
{
...
#ifdef WL_CFG80211
#ifdef WLMESH_CFG80211
static int wl_android_set_rsdb_mode(struct net_device *dev, char *command,
				    int total_len)
{
	int ret;
	...
	ret = wldev_iovar_setbuf(dev, "rsdb_mode", &rsdb_mode_cfg,
				 sizeof(rsdb_mode_cfg), smbuf, sizeof(smbuf),
				 NULL);
	if (ret < 0)
...
int wl_handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
	...
	if (strnicmp(command, CMD_STOP, strlen(CMD_STOP)) == 0) {
		...
		bytes_written =
			wl_android_set_csa(net, command, priv_cmd.total_len);
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
...
#ifdef WL_CFG80211
#ifdef WLMESH_CFG80211
	else if (strnicmp(command, CMD_SAE_SET_PASSWORD,
			  strlen(CMD_SAE_SET_PASSWORD)) == 0) {
		int skip = strlen(CMD_SAE_SET_PASSWORD) + 1;
		bytes_written = wl_cfg80211_set_sae_password(
			net, command + skip, priv_cmd.total_len - skip);
	} else if (strnicmp(command, CMD_SET_RSDB_MODE,
			    strlen(CMD_SET_RSDB_MODE)) == 0) {
		bytes_written = wl_android_set_rsdb_mode(net, command,
							 priv_cmd.total_len);
	}
#endif
#endif /* WL_CFG80211 */
		...
		bytes_written = wl_android_get_singlecore_scan(
		   ...
		   0) {
		bytes_written = wl_android_set_singlecore_scan(
			net, command, priv_cmd.total_len);
	}
#ifdef TEST_TX_POWER_CONTROL
		...
		wl_android_set_sarlimit_txctrl(net,
					       (const char *)command + skip);
	}
#endif /* CUSTOMER_HW4_PRIVATE_CMD */
		...
		bytes_written = wl_android_set_ulb_mode(net, command,
							priv_cmd.total_len);
	else if (strnicmp(command, CMD_ULB_BW, strlen(CMD_ULB_BW)) == 0)
		bytes_written =
			wl_android_set_ulb_bw(net, command, priv_cmd.total_len);
#endif /* WL11ULB */
		...
		bytes_written = wl_android_set_wfds_hash(net, command,
		...
		bytes_written =
			wl_tbow_teardown(net, command, priv_cmd.total_len);
	}
#endif /* BT_WIFI_HANDOVER */
#ifdef CUSTOMER_HW4_PRIVATE_CMD
#ifdef FCC_PWR_LIMIT_2G
		...
		bytes_written = wl_android_set_fcc_pwr_limit_2g(
			net, command, priv_cmd.total_len);
	}
#endif /* FCC_PWR_LIMIT_2G */
		...
		bytes_written = wl_android_stat_report_get_start(
			net, command, priv_cmd.total_len);
	} else if (strnicmp(command, CMD_STAT_REPORT_GET_NEXT,
			    strlen(CMD_STAT_REPORT_GET_NEXT)) == 0) {
		bytes_written = wl_android_stat_report_get_next(
			net, command, priv_cmd.total_len);
	}
#endif /* STAT_REPORT */
